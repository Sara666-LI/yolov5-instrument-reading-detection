...
def run(
    ...
    if len(stats) and stats[0].any():
        ...
        ##m_tp, m_fp= tp.mean(), fp.mean()
        nt = np.bincount(stats[3].astype(int), minlength=nc)  # number of targets per class
    else:
        nt = torch.zeros(1)

    # Print results
    pf = '%20s' + '%11i' * 4 + '%11.3g' * 4  # print format
    LOGGER.info(pf % ('all', seen, nt.sum(), tp.sum(), fp.sum(), mp, mr, map50, map))
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/myVOC_panel.yaml', help='dataset.yaml path')
